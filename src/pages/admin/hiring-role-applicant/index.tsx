...
const HiringRoleApplicant = () => {
    ...
    const fetchFunction = useCallback(async (page: number, pageSize: number) => {
        setFetching(true)
        try {
            const p: any = await axios.get(`${TEST_API_URL}/hiring-role-applicant/get?page=${page}&page_size=${pageSize}`, {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const response = await axios.get(`${TEST_API_URL}/hiring-role-applicant/get/${search}`, {
                withCredentials: true,
                headers: {
                    Authorization: `${localStorage.getItem('access_token')}`,
                    'Content-Language': `${localStorage.getItem('content-language')}`,
