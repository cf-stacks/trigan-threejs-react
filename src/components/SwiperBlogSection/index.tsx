              ...
              >
                <Link href={post.link}>
                  <a className="blockmb-2 ">
                    <Image
                      className="mb-2 max-h-56 w-full overflow-hidden rounded-lg object-cover shadow-sm"
                      alt="blog-images"
                      src={post.image}
                      width={500}
                      height={300}
                ...
                <div className="flex py-1">
                  {post.tags.tags.map((tag) => (
                    <div
                      key={post.tags.id}
                      className="flex items-center rounded-full bg-purple-500 px-3 py-1.5 text-xs font-medium uppercase leading-none text-white"
                    >
                      <span>{tag}</span>
                    </div>
                  ))}
                </div>
                <h2 className="py-2 text-lg font-bold sm:text-xl md:text-2xl">
                  <Link href={post.link}>
                    <a>{post.title}</a>
                  </Link>
                </h2>
